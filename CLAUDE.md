...
| **Infra** | `src/hooks-config.ts`, `src/push-store.ts`, `src/tunnel-manager.ts`, `src/image-watcher.ts`, `src/file-stream-manager.ts`, `src/attachment-registry.ts`, `src/attachment-magic.ts` | Attachments: see Key Patterns |
...
**Attachments** (live external document references, COD-37/PR #119): two pieces. `attachment-registry.ts` is an **in-memory** registry mapping a stable `attachmentId` → an absolute, `realpath`-resolved, extension-allowlisted file path, so browser requests (`GET /api/sessions/:id/attachments/:attachmentId/raw`) never carry arbitrary absolute paths; `POST /api/sessions/:id/attachments` registers one (both in `file-routes.ts`). `attachment-magic.ts` parses `codeman://attach?...` **magic links** out of terminal output — ⚠️ this scanner is prompt-injectable, so the scan path is **force-confined to the session workspace** (a hostile prompt could otherwise make it read arbitrary host files over SSE). Security gate is an extension **allowlist** (`isSupportedAttachmentExtension`), not a blocklist. Session-local files keep using the existing workspace-scoped `file-routes` paths; the registry is only for explicit live externals.
...
~140 handlers across 16 route files in `src/web/routes/`: system (41, incl. self-update `check`/`status`/`POST /api/system/update`, `POST /api/system/span-displays` → spawns `scripts/span-codeman.sh`, and `GET /api/codex/status`), sessions (29), orchestrator (10), cases (9), ralph (9), plan (8), files (8, incl. attachment register + `:attachmentId/raw`), respawn (7), mux (5), push (4), scheduled (4), teams (2), hooks (1), clipboard (1), status-telemetry (1, `POST /api/status-telemetry` ← statusLine exporter), ws (1 WebSocket). Each file has `@fileoverview` with endpoint details.
